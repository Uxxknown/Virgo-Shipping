...
export default function App() {
  ...
  const handleAuth = async ({ email, name, type }) => {
    ...
    try {
      if (type === 'register') {
        ...
        // Save to public user list for discovery/admin
        ...
        if (userDoc.exists()) {
          ...
          if (email.toLowerCase().includes('admin')) {
              ...
              alert("No account found for this session. Please Register first.");
              setPage('register');
          }
        }
      }
  ...
  const handlePreAlert = async (data) => {
    ...
    try {
      ...
      console.error("Error creating pre-alert:", err);
    }
  };
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
      ...
      <p className="text-slate-600 font-medium">Connecting to SwiftShip Cloud...</p>
    </div>
  );
    ...
    <div className="max-w-md mx-auto py-20 px-4">
      <div className="bg-white p-8 rounded-3xl shadow-xl">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onAuth({ email, name, type }); }} className="space-y-4">
          ...
          <button 
            disabled={isLoading}
            className="w-full bg-blue-600 text-white p-4 rounded-2xl font-bold hover:bg-blue-700 disabled:bg-blue-300 transition-all flex items-center justify-center"
          >
